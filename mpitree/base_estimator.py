...
class DecisionTreeEstimator:
    ...
    def __init__(self, criterion={}):
        ...
        optimal_threshold
            - a dictionary containing the best split for each continuous descriptive feature being tested
        """
        self.root = None
        self.n_levels = None
    ...
    def find_optimal_threshold(self, df, d):
        ...
        for threshold in thresholds:
            ...
            metric_left = self.metric(left.iloc[:, :-1], left.iloc[:, -1])
            metric_right = self.metric(right.iloc[:, :-1], right.iloc[:, -1])
        ...
        while not node.is_leaf:
            ...
            if is_numeric_dtype(query_branch):
                ...
                for child in node.children:
                    ...
                    raise ValueError(
                        f"Branch {node.feature} -> {query_branch} does not exist"
                    )
        return node
